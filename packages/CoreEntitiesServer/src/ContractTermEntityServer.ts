 ...
 *
 * THE ESCALATION CAP IS THE INTERESTING ONE, and as of 2026-08-05 it is a rule ACROSS ROWS.
 * `ContractTerm.MaxEscalationPercent` was removed, so the ceiling lives in exactly one place —
 * `ContractType.DefaultMaxEscalationPercent` — and a term's `EscalationPercent` is checked against
 * its contract's type rather than against a column beside it.
 *
 * Two consequences worth stating plainly, because they are the price of the simplification:
 *
 *  - The ceiling is now a property of the TYPE, not of the individual agreement. A term can no
 *    longer carry a separately-negotiated cap; changing one contract's ceiling means giving it a
 *    contract type that says so.
 *  - The rule applies to EXISTING terms too. The old per-term column let a term negotiated without
 *    a ceiling stay uncapped forever; with the cap resolved from the type, editing such a term now
 *    validates against the type's ceiling.
 *
 * It could not be a CHECK constraint even when both columns were local: CodeGen derives a generated
 ...
 * hit and documented. Now that the cap lives on another table it could never have been one.
 *
...
export class ContractTermEntityServer extends mjBizAppsContractsContractTermEntity {
    ...
    public override async ValidateAsync(): Promise<ValidationResult> {
        ...
        await this.checkEscalationUnderTypeCeiling(result);
        return result;
    }
    ...
    /**
     * The cap rule, resolved from the contract's TYPE.
     *
     * Both values are fractions (0.05 = 5%), and a type with no ceiling means "uncapped" —
     * deliberately permitted, because plenty of real agreements have no ceiling and pretending
     * otherwise would make them unrecordable.
     *
     * COSTS NOTHING WHEN IT CANNOT FIRE. A term with no escalation cannot exceed any ceiling, so the
     * common case — most terms — returns before reading anything. Only a term that actually escalates
     * pays for the one contract read, and the type itself comes from the engine's cache.
     */
    private async checkEscalationUnderTypeCeiling(result: ValidationResult): Promise<void> {
        const pct = this.EscalationPercent;
        if (pct === null || pct === undefined || !this.ContractID) return;

        const cap = await this.ceilingFromContractType();
        if (cap === null || cap === undefined || pct <= cap) return;

        result.Success = false;
        result.Errors.push(
            new ValidationErrorInfo(
                'EscalationPercent',
                `An escalation of ${(pct * 100).toFixed(2)}% exceeds the ceiling of ` +
                    `${(cap * 100).toFixed(2)}% set by this contract's type. Either lower the escalation or ` +
                    `move the contract to a type whose ceiling permits it — the ceiling is what the paper ` +
                    `says, so changing it is a negotiation, not a correction.`,
                pct,
            ),
        );
    }
    ...
    /**
     * The escalation ceiling for this term, via its contract's type. Null when the contract cannot be
     * read or its type sets no ceiling.
     *
     * The term knows its contract, not its type, so the type is resolved through it: one read for the
     * contract, then the cached type.
     */
    private async ceilingFromContractType(): Promise<number | null> {
        const contractTypeID = await this.contractTypeID();
        if (!contractTypeID) return null;
        return ContractsEngine.Instance.DefaultMaxEscalationFor(contractTypeID);
    }

    /** This term's contract's type, or null when the contract cannot be read. */
    private async contractTypeID(): Promise<string | null> {
        ...
        if (!res?.Success) {
            // A failed read must not silently skip the ceiling — a term quietly escalating past its
            // type's cap is exactly the outcome this rule exists to prevent.
            throw new Error(`Could not read the contract's type to check the escalation ceiling: ${res?.ErrorMessage ?? 'unknown error'}`);
        }
        return res.Results?.[0]?.ContractTypeID ?? null;
    }
