...
| R-20 | R-14's *"`ContractTemplateProvision.Sequence` **stays**: provision numbers do not sort as text"* | **Reversed — the column goes.** The premise is right and the conclusion does not follow: they do not sort as text *unpadded*, which a derived sort key fixes without a maintained column. The kept column has already failed at the job it was kept for — the seeded MA has `'1'` and `'1.1'` both at `Sequence = 1`, so the canonical order it exists to preserve is already ambiguous in live data. Order comes from a `ProvisionSortKey` in the app-owned layered view instead; proven over all 73 seeded rows to reproduce natural-sort order exactly. Detail + the padding rationale in `plans/backend-requirements.md` R-11 | Marcelo, 2026-08-19 |
