-- =============================================================================
-- BizAppsContracts: Add Predictive Contract Renewal Fields & Layered Base Views
-- Materialized prediction write-back columns and engineered operational features
-- computed via layered vwContracts wrapper for Predictive Studio renewal risk.
-- =============================================================================

---------------------------------------------------------------------------
-- 1. Contract: Add materialized prediction fields
---------------------------------------------------------------------------
IF NOT EXISTS (
    SELECT 1 FROM sys.columns 
    WHERE object_id = OBJECT_ID('[${flyway:defaultSchema}].[Contract]') 
      AND name = 'PredictedNonRenewalRisk'
)
BEGIN
...
IF NOT EXISTS (
    ...
    WHERE object_id = OBJECT_ID('[${flyway:defaultSchema}].[CK_Contract_PredictedRenewalRiskBand]')
)
BEGIN
    ALTER TABLE [${flyway:defaultSchema}].[Contract] ADD CONSTRAINT [CK_Contract_PredictedRenewalRiskBand]
        CHECK ([PredictedRenewalRiskBand] IS NULL OR [PredictedRenewalRiskBand] IN ('Low', 'Medium', 'High', 'Critical'));
END
GO

IF EXISTS (
    SELECT 1 FROM fn_listextendedproperty(N'MS_Description', 'SCHEMA', N'${flyway:defaultSchema}', 'TABLE', N'Contract', 'COLUMN', N'PredictedNonRenewalRisk')
)
    EXEC sp_updateextendedproperty @name = N'MS_Description', @value = N'Predicted probability (0.0000 to 1.0000) that this commercial agreement will fail to renew prior to the required notice deadline.', @level0type = N'SCHEMA', @level0name = N'${flyway:defaultSchema}', @level1type = N'TABLE', @level1name = N'Contract', @level2type = N'COLUMN', @level2name = N'PredictedNonRenewalRisk';
ELSE
    EXEC sp_addextendedproperty @name = N'MS_Description', @value = N'Predicted probability (0.0000 to 1.0000) that this commercial agreement will fail to renew prior to the required notice deadline.', @level0type = N'SCHEMA', @level0name = N'${flyway:defaultSchema}', @level1type = N'TABLE', @level1name = N'Contract', @level2type = N'COLUMN', @level2name = N'PredictedNonRenewalRisk';
GO

IF EXISTS (
    SELECT 1 FROM fn_listextendedproperty(N'MS_Description', 'SCHEMA', N'${flyway:defaultSchema}', 'TABLE', N'Contract', 'COLUMN', N'PredictedRenewalRiskBand')
)
    EXEC sp_updateextendedproperty @name = N'MS_Description', @value = N'Derived operational risk band classifying renewal risk: Low (<20%), Medium (20-50%), High (50-80%), Critical (>=80%).', @level0type = N'SCHEMA', @level0name = N'${flyway:defaultSchema}', @level1type = N'TABLE', @level1name = N'Contract', @level2type = N'COLUMN', @level2name = N'PredictedRenewalRiskBand';
ELSE
    EXEC sp_addextendedproperty @name = N'MS_Description', @value = N'Derived operational risk band classifying renewal risk: Low (<20%), Medium (20-50%), High (50-80%), Critical (>=80%).', @level0type = N'SCHEMA', @level0name = N'${flyway:defaultSchema}', @level1type = N'TABLE', @level1name = N'Contract', @level2type = N'COLUMN', @level2name = N'PredictedRenewalRiskBand';
GO

IF EXISTS (
    SELECT 1 FROM fn_listextendedproperty(N'MS_Description', 'SCHEMA', N'${flyway:defaultSchema}', 'TABLE', N'Contract', 'COLUMN', N'PredictedRenewalScoredAt')
)
    EXEC sp_updateextendedproperty @name = N'MS_Description', @value = N'Timestamp of the most recent predictive model scoring execution for this contract.', @level0type = N'SCHEMA', @level0name = N'${flyway:defaultSchema}', @level1type = N'TABLE', @level1name = N'Contract', @level2type = N'COLUMN', @level2name = N'PredictedRenewalScoredAt';
ELSE
    EXEC sp_addextendedproperty @name = N'MS_Description', @value = N'Timestamp of the most recent predictive model scoring execution for this contract.', @level0type = N'SCHEMA', @level0name = N'${flyway:defaultSchema}', @level1type = N'TABLE', @level1name = N'Contract', @level2type = N'COLUMN', @level2name = N'PredictedRenewalScoredAt';
GO

---------------------------------------------------------------------------
-- 2. Layered Views: vwContractsGenerated & vwContracts
---------------------------------------------------------------------------
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM [${mjSchema}].[EntityField] WHERE [EntityID] = @contractEntityID AND [Name] = 'PredictedNonRenewalRisk')
    BEGIN
        INSERT INTO [${mjSchema}].[EntityField] (
            ...
            '1de502c3-42f6-4246-b7a0-90f61c38d586', @contractEntityID,
            (SELECT COALESCE(MAX([Sequence]), 0) + 1 FROM [${mjSchema}].[EntityField] WHERE [EntityID] = @contractEntityID),
            'PredictedNonRenewalRisk', 'Predicted Non Renewal Risk',
    ...
    IF NOT EXISTS (SELECT 1 FROM [${mjSchema}].[EntityField] WHERE [EntityID] = @contractEntityID AND [Name] = 'PredictedRenewalRiskBand')
    BEGIN
        INSERT INTO [${mjSchema}].[EntityField] (
            [ID], [EntityID], [Sequence], [Name], [DisplayName], [Description], [Type], [Length], [Precision], [Scale],
            [AllowsNull], [DefaultValue], [AutoIncrement], [AllowUpdateAPI], [IsVirtual], [IsComputed],
            ...
            'e25c8e07-f5dc-4d15-9ef4-56f51111575e', @contractEntityID,
            (SELECT COALESCE(MAX([Sequence]), 0) + 1 FROM [${mjSchema}].[EntityField] WHERE [EntityID] = @contractEntityID),
            'PredictedRenewalRiskBand', 'Predicted Renewal Risk Band',
    ...
    IF NOT EXISTS (SELECT 1 FROM [${mjSchema}].[EntityField] WHERE [EntityID] = @contractEntityID AND [Name] = 'PredictedRenewalScoredAt')
    BEGIN
        INSERT INTO [${mjSchema}].[EntityField] (
            [ID], [EntityID], [Sequence], [Name], [DisplayName], [Description], [Type], [Length], [Precision], [Scale],
            [AllowsNull], [DefaultValue], [AutoIncrement], [AllowUpdateAPI], [IsVirtual], [IsComputed],
            ...
            'e2e2a570-28ca-4b84-b2c7-aa09d546fd72', @contractEntityID,
            (SELECT COALESCE(MAX([Sequence]), 0) + 1 FROM [${mjSchema}].[EntityField] WHERE [EntityID] = @contractEntityID),
            'PredictedRenewalScoredAt', 'Predicted Renewal Scored At',
    ...
    USING (
        SELECT 
            NEWID() AS ID,
            @contractEntityID AS EntityID,
            (SELECT ISNULL(MAX(Sequence), 0) FROM [${mjSchema}].[EntityField] WHERE EntityID = @contractEntityID) + 
